...
if (length(args)<6) {
        stop("Two first arguments must be cancer types ('Basal', 'TNBC' or 'LumA'), the third must be a valid p-value, the fourth must be a valid fold-change value and the fifth and sixth must be valid output relative paths for pdf and csv files.", call.=FALSE)
}

if (args[1] == args[2]) {
        stop("Cannot perform DEA with the same subsets.")
}
...
if(args[1] == "LumA"){d1 = brca_rnaseq.luminal}
if(args[2] == "Basal"){d2 = brca_rnaseq.basal}
if(args[2] == "TNBC"){d2 = brca_rnaseq.tnbc}
if(args[2] == "LumA"){d2 = brca_rnaseq.luminal}
...
df.l <- data.frame(sample = colnames(d1), status = rep(0, length(colnames(d1))))
df.t <- data.frame(sample = colnames(d2), status = rep(1, length(colnames(d2))))
df <- rbind(df.t,df.l)
...
diff.exp.df <- topTable(fit, coef = "status", n = Inf, sort = "p", p = 0.05) # Positive log-fold-changes mean higher expression in d1
...
write.csv(filter(tab2, abs(logFC) > lfc & negLogPval > -log10(pval)), paste0("../", args[6])) # write output

pdf(file = paste0("../", args[5]), width = 9, height = 4.5)
par(mar = c(5, 4, 4, 5))
